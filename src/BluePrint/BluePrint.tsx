...
const BluePrint = ({
  ...
  function getpattern(roll: number, pitch: number) {
    ...
    const rollLength = propsRef.current.pattern.length;

    const wrappedRoll =
      (((roll + rollOffset) % rollLength) + rollLength) % rollLength;

    return propsRef.current.pattern[wrappedRoll][pitch];
  }
  ...
  useEffect(() => {
    ...
    bluePrintMemo = document.getElementById(
      "bluePrintMemo"
    ) as HTMLCanvasElement;
    bluePrintMemo.addEventListener("mousedown", (event: any) => {
      selectFrame(event);
    });
    ...
    return () => {
      bluePrintMemo.removeEventListener("mousedown", (event: any) => {
        selectFrame(event);
      });
      console.log("おわり");
      cancel = true;
  ...
  async function drawPompom(
    ctx: any,
    canvas: HTMLCanvasElement,
    pompomCanvas: HTMLCanvasElement
  ) {
    ...
    for (let pitch = 0; pitch < pitchWidth; pitch++) {
      for (let roll = 0; roll < rollWidth; roll++) {
        try {
          ...
          ctx.lineTo(
            center.x + Math.cos(theta + thetaWidth) * r,
            center.y + Math.sin(theta + thetaWidth) * r
          );
          ctx.lineTo(
            center.x + Math.cos(theta + thetaWidth) * (r + rWidth),
            center.y + Math.sin(theta + thetaWidth) * (r + rWidth)
          );
          ctx.lineTo(
            center.x + Math.cos(theta) * (r + rWidth),
            center.y + Math.sin(theta) * (r + rWidth)
          );
          ctx.lineTo(
            center.x + Math.cos(theta) * r,
            center.y + Math.sin(theta) * r
  ...
  function drawFrame(
    ...
    // console.log(center)
    ctx.moveTo(
      center.x + Math.cos(getTheta(roll + 1)) * getR(pitch),
      center.y + Math.sin(getTheta(roll + 1)) * getR(pitch)
    );
    for (let i = 0; i <= widthCount; i += 1) {
      ctx.lineTo(
        center.x + Math.cos(getTheta(roll + 1 - i)) * getR(pitch),
        center.y + Math.sin(getTheta(roll + 1 - i)) * getR(pitch)
      );
    ...
    for (let i = widthCount; i >= 0; i -= 1) {
      ctx.lineTo(
        center.x + Math.cos(getTheta(roll + 1 - i)) * getR(pitch + piled),
        center.y + Math.sin(getTheta(roll + 1 - i)) * getR(pitch + piled)
      );
    }
  ...
  function frameShapeEx(ctx: any, frame: any, progress: number) {
    ...
    if ((frame.pitch + piled) % 2 == 0) {
      ...
      drawFrame(
        ctx,
        frame.center,
        roll + restroll - frame.widthCount,
        frame.pitch + piled,
        1,
        restroll
      );
  ...
  function selectFrame(event: any) {
    for (let i = 0; i < frames.length; i++) {
      ...
      let center_x =
        frame.center.x +
        Math.cos(getTheta(frame.roll + 1 - frame.widthCount / 2)) *
          getR(frame.pitch + frame.piled / 2);
      let center_y =
        frame.center.y +
        Math.sin(getTheta(frame.roll + 1 - frame.widthCount / 2)) *
          getR(frame.pitch + frame.piled / 2);
  ...
  useEffect(() => {
    console.log("メモ");
    let bluePrintMemo = document.getElementById(
      "bluePrintMemo"
    ) as HTMLCanvasElement;
  ...
  const rollCountSum = rollProgress.reduce(
  ...
  return (
    <>
      ...
      >
        ...
        <ImageSave
          data={{ pattern, colorList, rollWidth, pitchWidth }}
        ></ImageSave>
        {/* </div> */}
        ...
        <div
          ...
          id="allProgress"
        >
          残り{rollWidth * pitchWidth - rollCountSum}巻き　
          {((rollCountSum / (rollWidth * pitchWidth)) * 100).toFixed(0)}%
        </div>
        <Help id="allProgress" placement="bottom">
          全体の進捗
        </Help>
      </div>
      <div className="col-12 col-lg-6">
        ...
        >
          <RollCounter
            ...
            setRollProgress={setRollProgress}
          ></RollCounter>

          <Overlay
            target={() => document.getElementById("share")}
            show={completeTextEnable}
          >
            {(props) => (
              <Tooltip {...props}>
                おめでとうございます!
